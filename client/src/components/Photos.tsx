...
import { iVKPhoto } from '../VK.API/VK.Api.call';
...
export class Photos extends React.Component<iProps> {
    ...
    renderTemplate = () => {
        ...
        if (error) return <p className="error">Во время загрузки фото произошла ошибка</p>
        if (isFetching) return <p>Загрузка...</p>
        else return photos.map((photo, index) => ( // [1]
        ...
        return (
            <div className="ib page">
                <p>
                    ...
                    <button className="btn" onClick={this.onBtnClick}>2010</button>{' '}
